...
class FragmentQrCodeGenerator : Fragment(), OnBaseFragmentListener, View.OnClickListener {

    companion object {
        private val TAG = BuildConfig.BASE_TAG + ".FragmentQrCodeGenerator"
    }
    ...
    private val mQRCodeGeneratorResponseNetworkCallBack = object : NetworkCallBack<DTOQRCodeGeneratorResponse> {
        override fun onSuccess(dto : DTOQRCodeGeneratorResponse) {
            ...
            if (activity is OnBaseActivityListener) {
                val bundle = Bundle()
                bundle.putString(FragmentQrCode.EXTRA_QR_CODE_TITLE, if (PreferenceData.getUserType(activity).equals(Constants.USER_TYPE_MERCHANT)) {
                    PreferenceData.getLoginMerchantId(activity)
                } else {
                    PreferenceData.getLoginAgentId(activity)
                })
                bundle.putBoolean(FragmentQrCode.EXTRA_IS_DYNAMIC_QR_CODE, true)
                bundle.putString(FragmentQrCode.EXTRA_BILL_NUMBER, data !!.billNumber)
                bundle.putString(FragmentQrCode.EXTRA_BILL_AMOUNT, data.amount)
                bundle.putString(FragmentQrCode.EXTRA_QR_CODE_TOKEN, data.qrCodeToken)
                val fragment = FragmentProvider.getFragment(FragmentTag.QR_CODE)
                (activity as OnBaseActivityListener).onBaseActivityAddFragment(fragment !!, bundle, true, FragmentTag.QR_CODE)
            }
        }
    ...
    override fun onClick(view : View) {
        ...
        when (view.id) {
            R.id.fragment_qrcode_generator_textView_cancel -> activity.onBackPressed()
            R.id.fragment_qrcode_generator_textView_generate -> startQRCodeGeneratingProcess()
        }
    }
    ...
    private fun init() {
        ...
        view !!.findViewById<View>(R.id.fragment_qrcode_generator_textView_generate).setOnClickListener(this)
        view !!.findViewById<View>(R.id.fragment_qrcode_generator_textView_cancel).setOnClickListener(this)
        mTextInputLayoutBillNumber = view !!.findViewById<View>(R.id.fragment_qrcode_generator_textInputLayout_bill_number) as TextInputLayout
        mEditTextBillNumber = view !!.findViewById<View>(R.id.fragment_qrcode_generator_editText_bill_number) as EditText
        mTextInputLayoutBillDescription = view !!.findViewById<View>(R.id.fragment_qrcode_generator_textInputLayout_bill_description) as TextInputLayout
        mEditTextBillDescription = view !!.findViewById<View>(R.id.fragment_qrcode_generator_editText_bill_description) as EditText
        mTextInputLayoutBillAmount = view !!.findViewById<View>(R.id.fragment_qrcode_generator_textInputLayout_bill_amount) as TextInputLayout
        mEditTextBillAmount = view !!.findViewById<View>(R.id.fragment_qrcode_generator_editText_bill_amount) as EditText
    ...
    private fun startQRCodeGeneratingProcess() {
        ...
        val dtoQRCodeGeneratorRequest = DTOQRCodeGeneratorRequest(token !!, timeStamp, publicKey, PreferenceData.getUserType(activity), PreferenceData.getLoginMerchantId(activity), PreferenceData.getLoginAgentId(activity), billAmount, billNumber, billDescription)
