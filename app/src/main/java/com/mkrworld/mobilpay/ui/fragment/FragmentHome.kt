...
class FragmentHome : Fragment(), OnBaseFragmentListener, BaseViewHolder.VHClickable, ReadSmsTask.OnMessageFilteredListener {
    ...
    private val mFCMResponseNetworkCallBack = object : NetworkCallBack<DTOFCMResponse> {
        override fun onSuccess(dto : DTOFCMResponse) {
     ...
     * @return
     */
    private fun homeTabList() : ArrayList<BaseAdapterItem<*>> {
    ...
    override fun onViewHolderClicked(holder : BaseViewHolder<*>, view : View) {
        ...
        when (view.id) {
            R.id.item_home_parent -> if (view.tag != null && view.tag is DTOHomeTab) {
                val dtoHomeTab = view.tag as DTOHomeTab
                for (index : Int in 0 .. (mBaseAdapter !!.itemCount - 1)) {
                    if (mBaseAdapter?.getItem(index)?.getData() is DTOHomeTab) {
                        (mBaseAdapter?.getItem(index)?.getData() as DTOHomeTab).isFocused = false
                    }
                }
                dtoHomeTab.isFocused = true
                when (dtoHomeTab.tabType) {
                    ...
                    DTOHomeTab.TabType.DYNAMIC_QR_CODE -> if (activity is OnBaseActivityListener) {
                        ...
                        (activity as OnBaseActivityListener).onBaseActivityAddFragment(fragment !!, null, true, FragmentTag.QR_CODE_GENERATOR)
                    }
                    DTOHomeTab.TabType.CASH_COLLECT -> if (activity is OnBaseActivityListener) {
                        if (Utils.isMerchant(activity)) {

                        } else {
                            val fragment = FragmentProvider.getFragment(FragmentTag.CASH_COLLECT)
                            (activity as OnBaseActivityListener).onBaseActivityAddFragment(fragment !!, null, true, FragmentTag.CASH_COLLECT)
                        }
                    }
                    ...
                    DTOHomeTab.TabType.AEPS_COLLECT -> if (activity is OnBaseActivityListener) {
                        val fragment = FragmentProvider.getFragment(FragmentTag.AEPS_COLLECT)
                        (activity as OnBaseActivityListener).onBaseActivityAddFragment(fragment !!, null, true, FragmentTag.AEPS_COLLECT)
                    }
                    ...
                    DTOHomeTab.TabType.SEND_BILL -> if (activity is OnBaseActivityListener) {
                        val fragment = FragmentProvider.getFragment(FragmentTag.SEND_BILL)
                        (activity as OnBaseActivityListener).onBaseActivityAddFragment(fragment !!, null, true, FragmentTag.SEND_BILL)
                    }
                }
    ...
    private fun init() {
        ...
        mBaseAdapter?.updateAdapterItemList(homeTabList())
    ...
    private fun registerFCM() {
        Tracer.debug(TAG, "registerFCM : ")
        object : AsyncTask<Void, Void, String>() {

            override fun doInBackground(vararg voids : Void) : String? {
                Tracer.debug(TAG, "registerFCM : doInBackground: ")
                val refreshedToken = FirebaseInstanceId.getInstance().token
                Tracer.debug(TAG, "registerFCM : doInBackground: " + refreshedToken !!)
                return refreshedToken ?: ""
            }

            override fun onPostExecute(result : String?) {
                super.onPostExecute(result)
                Tracer.debug(TAG, "registerFCM : onPostExecute : " + result)
                if (result == null || result.trim().isEmpty()) {
                    return
                }
                val date = Date()
                val timeStamp = Utils.getDateTimeFormate(date, Utils.DATE_FORMAT)
                val token = Utils.createToken(activity, getString(R.string.endpoint_update_fcm), date)
                val publicKey = getString(R.string.public_key)
                val dtoAgentFCMRequest = DTOFCMRequest(token !!, timeStamp, publicKey, PreferenceData.getUserType(activity), PreferenceData.getLoginMerchantId(activity), PreferenceData.getLoginAgentId(activity), result.trim())
                mAppNetworkTaskProvider !!.setFcmId(activity, dtoAgentFCMRequest, mFCMResponseNetworkCallBack)
            }
